...
class DataMigration(st.StructureMigration):
    ...
    def generate_columns(self):
        ...
        if self.destiny_connection.type == 'sqlserver':
            columns = '\n'.join(self.destiny_create_table.splitlines()[11:-3]).replace('NOT NULL', '').replace('NULL', '')
        elif self.destiny_connection.type == 'oracle':
            columns = self.origin_structure[["COLUMN_NAME"]] + " PATH '$." + self.origin_structure[["COLUMN_NAME"]] + "'"
            columns = ',\n'.join(columns["COLUMN_NAME"])
